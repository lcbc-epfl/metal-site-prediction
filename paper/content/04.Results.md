...
The only two structures where a sodium is detected (2OKQ, 6KFN) both bind the sodium have at least 2 sidechain ligand atoms and only one backbone (2OKQ) or no backbone ligand atom. Canonical sodium binding sites e.g such as in PDB 4I0W with two backbone carbonyl and one asparagine sidechain have probabilities around 5 %. 
...
The RMSD for the maximum predicted probability point for Metal3D are XX, XX A, for AlphaFill it is:
