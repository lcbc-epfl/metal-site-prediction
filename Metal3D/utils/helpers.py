...
def get_all_protein_resids(pdb_file):
    ...
    return np.array(mapping['new_resid'])
...
def get_all_metalbinding_resids(pdb_file):
    ...
    return prot.get('resid', sel="name CA and resname HIS HID HIE HIP CYS CYX GLU GLH GLN ASP ASH ASN GLN MET")
...
def compute_average_p_fast(point, cutoff=1):
    ...
    p = 0
    nearest_neighbors,indices = tree.query(point,k=15, distance_upper_bound=cutoff, workers=1)
...
def write_cubefile(bb, pvalues, box_N, outname='Metal3D_pmap.cube'):
    ...
    with open(outname, 'w') as cube:
        ...
        angstromToBohr =1.89
        cube.write(f'    1   {bb[0][0]*angstromToBohr: .6f}  {bb[0][1]*angstromToBohr: .6f}   {bb[0][2]*angstromToBohr: .6f}\n')
        cube.write(f"{str(box_N[0]).rjust(5)}    1.890000    0.000000    0.000000\n")
        cube.write(f"{str(box_N[1]).rjust(5)}    0.000000    1.890000    0.000000\n")
        cube.write(f"{str(box_N[2]).rjust(5)}    0.000000    0.000000    1.890000\n")
...
def find_unique_sites(pvalues, grid, writeprobes=False, probefile='probes.pdb', threshold=5, p=0.75):
    ...
    if writeprobes:
        ...
        with open(probefile, 'w') as f:
            for i,site in enumerate(sites):
                 f.write(f'HETATM    1 ZN    ZN A {i:3}    {site[0][0]: 8.3f}{site[0][1]: 8.3f}{site[0][2]: 8.3f}  {site[1]:.2f}  0.0           ZN2+\n')




def show_map(pdb,p=0.5, show_sticks_all=False, show_sticks_metalbinding=True, show_probes=True, show_pdb_metals=True):
    ...
    if os.path.getsize('metal_'+pdb+'.cube') < 5000000:
        view.addVolumetricData(open('metal_'+pdb+'.cube', 'r').read(),"cube", {'isoval': p, 'color': "blue", 'opacity': 0.75})
    else:
        print('cube file is too big, not showing the map')
        print('Use the VMD programm to visualize the map or choose custom residues in close proximity')
    ...
    if show_probes:
        view.getModel(1).setStyle({'resn':"ZN"},{'sphere': {'colorscheme':{'prop':'index', 'map':colors}}})
